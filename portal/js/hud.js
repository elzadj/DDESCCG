    ...
    var
        ...
        init = function () {
            if ($batteryLevel.length) {
                getBatteryStatus();
                intervals.device = setInterval(getBatteryStatus, 5000);
            }

            if ($connectivity.length) {
                updateConnectivity();
                intervals.connectivity = setInterval(updateConnectivity, 1000);
            }
        ...
        getBatteryStatus = function () {
            //console.log('func: getBatteryStatus()');
        ...
        updateConnectivity = function () {
            var status       = navigator.onLine ? 'on' : 'off',
